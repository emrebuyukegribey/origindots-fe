...
function SingleSelectField({ proper, properValueList, deleteProper, editProper }) {
  console.log("proper value list : ", properValueList)
  const [value, setValue] = useState(properValueList?.filter(p => p.properId === proper.id)[0].name);
  ...
  return (
    <div className="single-select-field-outer-container">
      ...
      <div className="single-select-field-container">
        ...
        {properValueList?.filter(p => p.properId === proper.id).map(prop => {
          return (
            <Radio.Group value={value} onChange={onChange}>
              <Radio size="large" value={prop.name}>{prop.name}</Radio>
            </Radio.Group>
            )
        })}
